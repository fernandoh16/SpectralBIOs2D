...
int main(int argc, char* argv[]) {
	...
	*/
    ...
    // === Integrand for BI === 
    auto Integrand = [&BI](std::vector<double> y, int level){
        return BI(y) ;
    } ;
    ...
    */
	...
	gMLQMC::samplers::Halton<double> IPL_Sampler(NumberOfParameters);
    int Me   = pow(2, Level + 1) ;
    auto Out = gMLQMC::SLQMC<gMLQMC::strategy::serial>(Integrand,IPL_Sampler,Me, Level) ;
    ...
    for(int i = 0; i<NumberOfControlPoints; i++) {
    	PriorOutput << fixed << setprecision(20) << Exp1[2 * i] << "   " << Exp1[2 * i + 1] << "\n" ; 
	}
    VD Exp2 =  get<1>(Out);
   	for(int i = 0; i<NumberOfControlPoints; i++) {
    	PosteriorOutput << fixed << setprecision(20) << Exp2[2 * i] / get<2>(Out) << "   " << Exp2[2 * i + 1] / get<2>(Out) << "\n" ; 
    }
    // cout << get<2>(Out)  << "\n" ;
    PriorOutput.close();
